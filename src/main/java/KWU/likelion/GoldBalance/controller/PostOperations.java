...
//    ResponseEntity<PostList> getPostsSortedByCommentCount();
    @GetMapping("/hot") //처음에 모든 정보를 받아 투표의 list를 반환함 - 투표 순
    ResponseEntity<PostList> getPostsSortedByVoteCount(@RequestParam Integer cursor);

    @GetMapping("/latest") //처음에 모든 정보를 받아 투표의 list를 반환함 - 시간 순
    ResponseEntity<PostList> getPostsSortedByTime(@RequestParam Integer cursor);
    @GetMapping("/balance") // 5퍼 내로 차이나는 게시물을 불러올 수 있는 메서드
    ResponseEntity<PostList> getPostsbyCloseVote(@RequestParam Integer cursor);
